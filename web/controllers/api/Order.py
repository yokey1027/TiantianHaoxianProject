...
def orderCallback():
	...
	app.logger.info( callback_data  )#取出数据
	# 将取出的sign值和返回的sign进行对比，看是否为伪造的sign值
...
'''
web模拟支付回调方法
所有回调基本都是校验合法性 然后找到订单id，模拟就是直接传递 xxx表示订单id
api/order/callback2?id=xxx
'''
